...
def admin_home():
    ...
    for quiz in quizzes:
        ...
        quiz_growth[date] = quiz_growth.get(date, 0) + 1  # More concise way to update count

    labels = list(quiz_growth.keys())
    values = list(quiz_growth.values())

    quiz_growth_data = {"labels": labels, "values": values}
    ...
    for usr in users:
        ...
        quiz = Quiz.query.filter_by(id=scores[0].quiz_id).first()
        if not quiz:
            continue
    ...
    quiz_growth_data=quiz_growth_data,
...
def users():
    try:
        ...
        users = User.query.filter_by(user_type='user').offset(skip).limit(take).all()
        final_list = []
        for usr in users:
            single_obj={}
        ...
        total_rows = User.query.filter_by(user_type='user').count()
